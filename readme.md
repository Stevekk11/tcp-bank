...
Server naslouchá na IP adrese 0.0.0.0 a port si nastavíte v konfiguraci. Výchozí port je 65525. Nezapomeňte ho změnit na školní port pokud bude potřeba.

Pro připojení k serveru můžete použít libovolného TCP klienta, například `putty` nebo `netcat` na Linuxu. Telnet raději nepoužívejte, protože nemusí správně zpracovávat české znaky.
...
*   **Účty**: Data jednotlivých účtů jsou uložena v adresáři `accounts/`. Každý soubor je pojmenován podle čísla účtu a obsahuje číselnou hodnotu zůstatku.
*   **Logy**: Záznamy o běhu serveru, připojených klientech a chybách se ukládají do adresáře `logs/` a vypisují se do konzole.
